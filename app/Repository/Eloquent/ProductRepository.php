...
class ProductRepository extends BaseRepository implements ProductRepositoryInterface
{
    ...
    public function all()
    {
        ...
        $sql = 'select 
                ...
                p.price_sell, 
                sum(pz.quantity) as quantity 
                from products p 
                join categories c on c.id = p.category_id
                join products_color pc on p.id = pc.product_id 
                join products_size pz on pz.product_color_id = pc.id
                where p.deleted_at is null 
                and pc.deleted_at is null 
                and pz.deleted_at is null 
                and c.deleted_at is null 
                group by p.id, p.name, p.img, category_name, p.price_sell
                order by p.id DESC 
                ';
    ...
    {
        $sql = 'select sum(order_details.quantity) as sum, products.id, products.name, products.price_sell, products.img from products 
